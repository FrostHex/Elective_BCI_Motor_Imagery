...
class CSPLDAClass:
    ...
    def recognize(self, data, personID):
        ...
        for segment in segments:
            data_csp_12 = self.fbcsp_transform(np.expand_dims(segment, 0), csp_12)
            data_csp_13 = self.fbcsp_transform(np.expand_dims(segment, 0), csp_13)
            data_csp_23 = self.fbcsp_transform(np.expand_dims(segment, 0), csp_23)
