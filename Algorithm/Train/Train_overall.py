...
class Train():
    ...
    def __init__(self):
        ...
        self.data = {id: {'left': None, 'right': None, 'feet': None} for id in range(1, 6)}
    ...
    def run(self):
        ...
        for id in range(1, 6):  # 几位受试者分别进行
            for task in ['12', '13', '23']:  # 12、13、23三个CSP的训练分别进行
                ...
                Y1_prime = self.W.T @ self.data[id][INTERPRET_TASK[x1]] # KxL @ LxN = KxN
                Y2_prime = self.W.T @ self.data[id][INTERPRET_TASK[x2]]
                # Y1_prime_fft = np.fft.fft(Y1_prime, axis=1)
                # Y2_prime_fft = np.fft.fft(Y2_prime, axis=1)

                k = Y1_prime_fft.shape[0]
                for i in range(k):
                    plt.figure()
                    plt.plot(Y1_prime_fft[i], label='Y1_prime_fft', color='blue')
                    plt.plot(Y2_prime_fft[i], label='Y2_prime_fft', color='orange')
    ...
    def test_lda(self, id, task):
        ...
        for block in range(self.valid_group_num[id] - 5, self.valid_group_num[id]):
            ...
            feature_1 = np.diag(np.cov(Y1_prime))
            feature_2 = np.diag(np.cov(Y2_prime))
            X = np.vstack((feature_1, feature_2))
        ...
        if operation == 'w':  # 保存三类样本的协方差矩阵至pkl文件
            ...
            for id in range(1, ID_NUM + 1):
                for block in range(1, BLOCK_NUM + 1):
                    ...
                    # 将数据按照trigger分类
                    self.data[id]['left'] = np.hstack((self.data[id]['left'], data[:L, data[64, :] == 201]))
                    self.data[id]['right'] = np.hstack((self.data[id]['right'], data[:L, data[64, :] == 202]))
                    self.data[id]['feet'] = np.hstack((self.data[id]['feet'], data[:L, data[64, :] == 203]))
            # 将数据转换为三类样本的协方差矩阵
            for id in range(1, ID_NUM + 1):
                for task in ['left', 'right', 'feet']:
                    if self.data[id][task].size > 0:
                        ...
                        self.cov_matrix_task_all[id][task] = np.zeros((L, L))
